import { Dataset, createPlaywrightRouter, RequestQueue } from 'crawlee';
...
router.addDefaultHandler(async ({ request, page, log }) => {
    ...
    try {
        ...
        console.log('Scraped Data:', JSON.stringify(apiResponse.data, null, 2));
        ...
        if (request.retryCount < maxRetries) {
            log.info(
                `Retrying request (${request.retryCount + 1}/${maxRetries})`,
            );
